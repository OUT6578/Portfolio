...
const portfolioData = {
  personalInfo: {
    ...
    avatar: profilePic,
    summary: "Associate React.js Developer with 1 year of experience crafting responsive, user-centric web applications. Expert in modern React.js, TypeScript, and state management, integrating REST APIs and optimizing performance in Agile environments."
  },
  skills: {
    technical: ["React.js", "TypeScript", "JavaScript", "Tailwind CSS", "Shadcn UI", "Redux Toolkit", "Node.js", "MongoDB", "Express.js", "HTML5/CSS3"],
    tools: ["VS Code", "Swagger", "npm packages", "Git/GitHub", "Postman", "MongoDB Compass"]
  },
  experience: [
    {
  ...
  projects: [
    {
      ...
      description: "LMS tailored for DFCCIL employees featuring role-based access, multi-level approvals, training history, and vendor management. Implemented core features like Training Application and Recommendations."
    },
    {
      title: "Task Management System",
      tech: "React, JS, Tailwind, Shadcn UI",
      description: "Task delegation platform for DFCCIL employees with hierarchy-based assignment constraints. Contributed to API Integration and UI/UX design, implementing extension and delegation workflows."
    },
    {
      title: "Visitor Management System",
      tech: "React, JS, Tailwind, Shadcn UI",
      description: "Digital entry tracking system. Focused on Dashboard API Integration and stability improvements through rigorous bug fixing."
    }
  ],
  education: [
    {
...
const TypingEffect = ({ text, speed = 100, delay = 0 }: { text: string; speed?: number; delay?: number }) => {
  ...
  return <span className="inline-block">{displayText}<span className="animate-pulse text-cyan-400">|</span></span>;
};
...
const ContactDialog = ({ isOpen, onClose, data }: { isOpen: boolean; onClose: () => void; data: typeof portfolioData.personalInfo }) => {
      ...
      >
        ...
        <div className="p-6 pt-12 flex flex-col items-center text-center relative">
          ...
          <div className="w-full space-y-3">
            <a href={`tel:${data.mobile}`} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors group">
              ...
              <div className="text-left flex-1">
                <p className="text-xs text-slate-500 uppercase font-semibold">Mobile</p>
                <p className="text-sm text-slate-200">{data.mobile}</p>
              </div>
            </a>
            
            <a href={`mailto:${data.email}`} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors group">
            ...
            <a href={data.website} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors group">
               ...
               <div className="text-left flex-1">
                ...
                <p className="text-sm text-slate-200 truncate">{data.website.replace(/^https?:\/\//, '')}</p>
              </div>
              <ExternalLink className="h-4 w-4 text-slate-600 group-hover:text-white transition-colors" />
            </a>
            
             <a href={data.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors group">
               ...
               <div className="text-left flex-1">
                ...
                <p className="text-sm text-slate-200 truncate">Connect on LinkedIn</p>
              </div>
              <ExternalLink className="h-4 w-4 text-slate-600 group-hover:text-white transition-colors" />
            </a>
          </div>
...
const handleDownloadCV = () => {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
    ...
    return (
      ...
      >
        {children}
        <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-cyan-400 transform transition-transform duration-300 origin-left ${
  ...
  return (
    <div className="min-h-screen font-sans text-slate-100 selection:bg-cyan-500/30 selection:text-cyan-100">
        ...
        <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-8 items-center text-sm font-medium">
            {['home', 'experience', 'projects', 'skills', 'education'].map((item) => (
              <NavLink key={item} href={`#${item}`}>{item.charAt(0).toUpperCase() + item.slice(1)}</NavLink>
            ))}
          </nav>

          <div className="flex items-center gap-4">
             <Button onClick={handleDownloadCV} size="sm" className="hidden sm:flex bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white border-0 shadow-lg shadow-cyan-500/20 transition-all hover:scale-105">
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="md:hidden text-slate-100 hover:bg-slate-800/50 active:scale-95 transition-all">
                  <Menu className="h-6 w-6" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="bg-slate-950/95 backdrop-blur-xl border-l border-slate-800 text-slate-100 w-[300px] sm:w-[400px]">
                <div className="flex flex-col h-full">
                  ...
                  <div className="mt-auto pb-8 px-4">
                     <Button onClick={handleDownloadCV} className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 shadow-lg shadow-cyan-900/20">
      ...
      <main className="container mx-auto px-4 md:px-6 pt-20 pb-12 space-y-16 relative z-10">
        ...
        {/* Hero Section */}
        <section id="home" className="min-h-[80vh] flex flex-col-reverse lg:flex-row items-center justify-center gap-8 lg:gap-16 py-10 lg:py-0">
            ...
            <RevealOnScroll>
              <div className="space-y-4">
                ...
                <h2 className="text-xl sm:text-2xl md:text-3xl text-slate-400 font-light h-8 md:h-10">
                  <TypingEffect text={portfolioData.personalInfo.title} speed={80} delay={500} />
                </h2>
              </div>
            </RevealOnScroll>
            ...
            <RevealOnScroll className="delay-300">
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4 px-8 sm:px-0">
                ...
                <div className="flex justify-center gap-3">
                  <Button size="icon" variant="outline" className="w-12 h-12 rounded-full border-slate-700 bg-slate-900/50 hover:bg-slate-800 hover:text-cyan-400 hover:border-cyan-400 transition-all">
                    <a href={portfolioData.personalInfo.linkedin} target="_blank" rel="noreferrer" className="flex items-center justify-center w-full h-full"><Linkedin className="h-5 w-5" /></a>
                  </Button>
                  <Button size="icon" variant="outline" className="w-12 h-12 rounded-full border-slate-700 bg-slate-900/50 hover:bg-slate-800 hover:text-cyan-400 hover:border-cyan-400 transition-all">
                    <a href={portfolioData.personalInfo.github} target="_blank" rel="noreferrer" className="flex items-center justify-center w-full h-full"><Github className="h-5 w-5" /></a>
                  </Button>
                </div>
              </div>
            </RevealOnScroll>
        ...
        <section id="experience" className="space-y-8">
          <RevealOnScroll>
            <div className="flex flex-col items-center mb-8">
              <Badge variant="outline" className="mb-4 border-cyan-500/30 text-cyan-400 px-4 py-1">Career Path</Badge>
                ...
                <Card className="bg-slate-900/40 border-slate-800 backdrop-blur-sm hover:border-cyan-500/30 transition-all hover:shadow-2xl hover:shadow-cyan-900/10 group">
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center text-xs md:text-sm text-slate-500">
                      <MapPin className="w-3 h-3 md:w-4 md:h-4 mr-1 text-cyan-600" /> {job.location}
                    </div>
                    <ul className="space-y-3">
                      {job.description.map((item, i) => (
                        <li key={i} className="flex items-start text-sm md:text-base text-slate-300 group/item">
        ...
        <section id="projects" className="scroll-mt-24">
          <RevealOnScroll>
             <div className="flex flex-col items-center mb-12">
              <Badge variant="outline" className="mb-4 border-purple-500/30 text-purple-400 px-4 py-1">Portfolio</Badge>
              <h2 className="text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-slate-500">
                Featured Projects
              </h2>
            </div>
        ...
        <section id="skills" className="py-8">
          <RevealOnScroll>
            <div className="flex flex-col items-center mb-12">
              <Badge variant="outline" className="mb-4 border-blue-500/30 text-blue-400 px-4 py-1">Expertise</Badge>
              <h2 className="text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-slate-500">
                Technical Arsenal
              </h2>
            </div>
        ...
        <section id="education" className="pb-12">
          <RevealOnScroll>
            <div className="flex flex-col items-center mb-12">
              <Badge variant="outline" className="mb-4 border-emerald-500/30 text-emerald-400 px-4 py-1">Academic</Badge>
              <h2 className="text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-slate-500">
                Education
              </h2>
            </div>
      ...
      <ContactDialog isOpen={isContactOpen} onClose={() => setIsContactOpen(false)} data={portfolioData.personalInfo} />
